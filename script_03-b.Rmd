...
```{r, s1a}
#increase
sep_diff <- left_join(sep_old, sep_new, by = "id", suffix = c(".old", ".new")) %>%
  mutate(u_grad_diff = (u_grad.new - u_grad.old))
sep_diff %>%
  arrange(desc(u_grad_diff)) %>%
  head(5) %>%
  select(title.old, name.old, u_grad.new, u_grad_diff) %>%
  rename("Title"= name.old, "Number" = title.old, "Enrollment" = u_grad.new, "Change" = u_grad_diff) %>%
  gt()
```
